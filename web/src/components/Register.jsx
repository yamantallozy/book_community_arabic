...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const msg = err.response?.data?.msg || 'فشل إنشاء الحساب';
            setError(msg);
            alert('خطأ في التسجيل: ' + msg); // POPUP DEBUGGING
        }
    };
        ...
        <div className="min-h-[calc(100vh-80px)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-lg border border-slate-100">
                <div className="text-center">
                    <h2 className="text-3xl font-bold text-slate-800">إنشاء حساب</h2>
                    <p className="mt-2 text-sm text-slate-500">انضم إلى مجتمع محبي الكتب</p>
                </div>
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div className="text-center">
                        <p className="text-sm text-slate-500">
                            لديك حساب بالفعل؟ <Link to="/login" className="font-medium text-primary hover:text-indigo-500">تسجيل الدخول</Link>
                        </p>
                    </div>
                </form>
            </div>
